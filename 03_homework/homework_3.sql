...
SELECT customer.customer_id, customer.customer_last_name, customer.customer_first_name, sum(customer_purchases.quantity*customer_purchases.cost_to_customer_per_qty) as money_spent
FROM customer
...
-- Step 1: Create the temp table from the vendor table
...
SELECT
    customer_id,
    sum(quantity*cost_to_customer_per_qty) AS money_spent_Apr2019
FROM customer_purchases
